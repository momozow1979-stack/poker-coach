...
void main() {
  ...
  group('initialAssignments / applyAssignments', () {
    ...
    test('リバーだけあってターンが無いと警告になる', () {
      final assignments = [
        AssignedCard(
          card: PlayingCard.parse('Ah'),
          confidence: 1,
          slot: HandSlot.hero,
        ),
        AssignedCard(
          card: PlayingCard.parse('Js'),
          confidence: 1,
          slot: HandSlot.hero,
        ),
        AssignedCard(
          card: PlayingCard.parse('Qs'),
          confidence: 1,
          slot: HandSlot.flop,
        ),
        AssignedCard(
          card: PlayingCard.parse('7d'),
          confidence: 1,
          slot: HandSlot.flop,
        ),
        AssignedCard(
          card: PlayingCard.parse('2c'),
          confidence: 1,
          slot: HandSlot.flop,
        ),
        AssignedCard(
          card: PlayingCard.parse('Ts'),
          confidence: 1,
          slot: HandSlot.river,
        ),
      ];
